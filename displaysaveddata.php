<?php 
    echo"<h2 class='disHis'>Saved Data of Kollam</h2>";?>
    <div class="his">
        <?php
        ...
        $display="SELECT* FROM weatherdata where city='Kollam' AND wdate>='$befSeven' AND wdate<'$currentDate' ORDER BY wdate ";
        $citydata=mysqli_query($conn,$display);
        if (mysqli_num_rows($citydata)>0){
            while ($cityrow=mysqli_fetch_assoc($citydata)){
                echo"<div style='background-color:#000000;float:left;padding:3px;border-radius:10px;margin-left:10px;'>";
